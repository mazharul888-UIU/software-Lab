      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(320px,.86fr)_minmax(0,1.35fr)]">
        <aside className="space-y-4">
          <div className="panel p-5">
            ...
            <div className="mt-4 space-y-3">{searchResults.map((student) => <PersonResult key={student.student_id} student={student} onConnect={sendRequest} onAccept={(id) => respondToRequest(id, "accept")} onDecline={(id) => respondToRequest(id, "decline")} onCancel={cancelRequest} onMessage={selectConnection} busyId={busyStudentId || busyRequestId} />)}</div>
          </div>
          ...
          {selectedConnection && <>
            <header className="clay-chat-header flex items-center gap-3 border-b border-ink/[0.07] bg-white/45 px-5 py-4 dark:bg-white/[0.025]">
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button onClick={() => setProfileOpen((open) => !open)} className="btn-ghost min-h-9 px-2 text-xs text-cobalt" aria-expanded={profileOpen} aria-controls="connection-profile">Profile</button>
